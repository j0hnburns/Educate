//
//  TeachingCollaborativeStrategiesViewController.m
//  Educate
//
...
@implementation TeachingCollaborativeStrategiesViewController
...
- (void)viewDidLoad {
	...
	customNavHeader.viewHeader.text = @"Collaborative Strategies";
	customNavHeader.viewHeader.font = [UIFont boldSystemFontOfSize:18];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	teachingStrategyDescriptionController.exampleButton.hidden = YES;
...
- (void)initialiseLocalStrategiesArray {
    ...
    if (sqlite3_open([path UTF8String], &educateDatabase) == SQLITE_OK) {
        ...
        const char *sql = "SELECT strategyID, strategyType, strategyName, exampleText, imageName, description FROM teachingStrategies WHERE strategyType = 2 ORDER BY strategyID";
	...
	if ([localStrategiesArray count] == 8) {
		...
		if (sqlite3_open([path UTF8String], &educateDatabase) == SQLITE_OK) {
			// Get the primary key for all books.
			const char *sql = "insert into teachingStrategies (strategyID, strategyType, strategyName, imageName) values (21, 2, 'Fishbowl', 'cs_fishbowl.png')";
			sqlite3_stmt *statement;
			// Preparing a statement compiles the SQL query into a byte-code program in the SQLite library.
			// The third parameter is either the length of the SQL string or -1 to read up to the first null terminator.        
			if (sqlite3_prepare_v2(educateDatabase, sql, -1, &statement, NULL) == SQLITE_OK) {
		...
		if (sqlite3_open([path UTF8String], &educateDatabase) == SQLITE_OK) {
			// Get the primary key for all books.
			const char *sql = "insert into teachingStrategies (strategyID, strategyType, strategyName, imageName) values (22, 2, 'Jigsaw', 'cs_jigsaw.png')";
			sqlite3_stmt *statement;
			// Preparing a statement compiles the SQL query into a byte-code program in the SQLite library.
			// The third parameter is either the length of the SQL string or -1 to read up to the first null terminator.        
			if (sqlite3_prepare_v2(educateDatabase, sql, -1, &statement, NULL) == SQLITE_OK) {
